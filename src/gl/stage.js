...
export class Stage {
  ...
  render(state, time, clips, heroClips) {
    ...
    if (heroClips && heroClips.length > 0) {
      const texs = [this.tex.hero0, this.tex.hero1, this.tex.hero2];
      let offsets = [0];
      if (heroClips.length === 3) {
        offsets = [0, -this.layout.w * 0.25, this.layout.w * 0.25];
      } else if (heroClips.length === 2) {
        offsets = [-this.layout.w * 0.15, this.layout.w * 0.15];
      }

      for (let c = 0; c < heroClips.length; c++) {
        const clip = heroClips[c];
        const offset = offsets[c] || 0;
        const texSlot = texs[c] || texs[0];
        const geo = this._heroGeometry(clip, state.hero, offset);
        this._heroShadow(geo, state.hero, heroSkew);
        this._heroFigure(clip, geo, state.hero, heroSkew, texSlot);
      }
    } else if (clips.hero) {
      const geo = this._heroGeometry(clips.hero, state.hero, 0);
      this._heroShadow(geo, state.hero, heroSkew);
      this._heroFigure(clips.hero, geo, state.hero, heroSkew, this.tex.hero0);
    }
   ...
   */
  _heroGeometry(clip, st, offsetX = 0) {
    ...
    return {
      wantH,
      offsetPx: offsetX * dpr,
      rect: [
        hero.cx * dpr - quadW * subCX + offsetX * dpr,
  ...
  _heroShadow(geo, st, skew) {
    ...
    this._quad(p, [hero.cx * dpr - sw / 2 + skew[0] * 0.55 + (geo.offsetPx || 0),
    hero.feet * dpr - sh, sw, sh]);
  }

  _heroFigure(clip, geo, st, skew, texSlot) {
    if (!geo) return;
    const gl = this.gl;
    if (clip.poll()) upload(gl, texSlot, clip.el);

    const p = this.progs.figure;
    gl.useProgram(p.p);
    gl.uniform1i(p.u.uClip, bind(gl, texSlot, 0));
    gl.uniform1i(p.u.uGrunge, bind(gl, this.tex.grunge, 1));
